...
export default function Kiosk() {
  ...
  const { setSelectedCategory, filteredRestaurant } = useFilteredRestaurant(
    '전체',
    restaurants,
    shopName,
  );
  ...
  const onClickOrder = () => {
    order(carts, totalPrice).then((res: ReceiptProps) => setReceipt(res));
  };
  ...
  return (
    <>
      ...
      <Carts
        ...
        setCarts={setCarts}
      />
      <SearchInput shopName={shopName} setShopName={setShopName} />
      <CategoryButtons
      ...
      <RestaurantList
        ...
        addCart={addCart}
      />
      <Receipt receipt={receipt} endReceipt={endReceipt} />
      <div style={{ color: '#979292' }}>영수증 나오는 곳</div>
    </>
  );
}
